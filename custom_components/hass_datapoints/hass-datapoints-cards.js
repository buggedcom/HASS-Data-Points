...
<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types">
  ...
  class HistoryAnnotationDialogController {
    ...
    _shake() {
      ...
      void this._dialogEl.offsetWidth;
...
`;
  ...
  function computeHistoryAnalysisInWorker(payload, options = {}) {
  ...
  class HassRecordsHistoryCard extends ChartCardBase {
    ...
    async _drawChart(histResult, statsResult, events, t0, t1, options = {}) {
      ...
      const onAnalysisProgress = analysisEntityIds.length ? (progress) => {
        this.dispatchEvent(new CustomEvent("hass-datapoints-analysis-computing", {
          bubbles: true,
          composed: true,
          detail: { computing: true, entityIds: analysisEntityIds, progress }
        }));
      } : null;
      const onAnomalyPartial = analysisEntityIds.length ? (entityId, method, clusters) => {
        ...
        this.dispatchEvent(new CustomEvent("hass-datapoints-analysis-method-result", {
          bubbles: true,
          composed: true,
          detail: { entityId, method, clusters }
        }));
      } : null;
      ...
      if (analysisEntityIds.length) {
        this.dispatchEvent(new CustomEvent("hass-datapoints-analysis-computing", {
          bubbles: true,
          composed: true,
          detail: { computing: false, entityIds: analysisEntityIds, progress: 100 }
        }));
      }
    ...
    _drawRecordedEventPoints(renderer, series, events, t0, t1, options = {}) {
      ...
      for (const event of events) {
        ...
        const y2 = hasNumericTarget ? renderer.yOf(value, targetSeries.axis.min, targetSeries.axis.max) : linkedToOtherTarget ? renderer.pad.top + renderer.ch : renderer.pad.top + 12;
...
`;
  ...
  class DpAnalysisAnomalyGroup extends i$2 {
    ...
    constructor() {
      ...
      this.computingMethods = /* @__PURE__ */ new Set();
    }
    _emit(key, value) {
      this.dispatchEvent(
        new CustomEvent("dp-group-analysis-change", {
...
`;
  class DpTargetRowList extends i$2 {
    ...
    render() {
      ...
      const allAnalysisSame = this.rows.every((r2) => JSON.stringify(r2.analysis ?? {}) === firstAnalysis);
      return b`
      <div class="history-target-table">
        <div
        ...
        (row, index) => b`
              <dp-target-row
                ...
                .computingMethods=${this.computingMethodsByEntity?.get(row.entity_id) ?? /* @__PURE__ */ new Set()}
...
`;
  class HassRecordsHistoryPanel extends HTMLElement {
    constructor() {
      ...
      this._computingEntityIds = /* @__PURE__ */ new Set();
      this._analysisProgress = 0;
      this._computingMethods = /* @__PURE__ */ new Map();
    ...
    _renderSidebarOptions() {
      ...
      this._sidebarOptionsComp.anyAnomaliesEnabled = (this._seriesRows ?? []).some((r2) => r2.analysis?.show_anomalies === true);
