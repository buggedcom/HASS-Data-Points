...
export class HassRecordsStatisticsCard extends LitElement {
  ...
  @state() accessor _chartMessage: string = "";

  @state() accessor _series: SeriesItem[] = [];

  @state() accessor _eventCount: number = 0;
  ...
  setConfig(config: CardConfig) {
    if (!config.entity && !config.entities) {
      throw new Error(
        "hass-datapoints-statistics-card: define `entity` or `entities`"
      );
    }
    this._config = {
      hours_to_show: 24,
  ...
  private _setupResizeObserver() {
    ...
    this._resizeObserver = new ResizeObserver(() => {
      if (Array.isArray(this._lastDrawArgs) && this._lastDrawArgs.length) {
        (this._drawChart as (...args: unknown[]) => void)(
          ...this._lastDrawArgs
        );
      }
    });
  ...
  private get _statIds(): string[] {
    ...
    const addId = (value: unknown) => {
      ...
      const resolved =
        typeof value === "string"
          ? value
          : obj?.entity || obj?.entity_id || obj?.statistics_id || "";
      if (resolved) {
        ids.push(resolved as string);
      }
    };
  ...
  async _load() {
    ...
    const maybeDraw = () => {
      ...
      const hasDrawableData = this._hasDrawableStatisticsData(
        partial.statsResult || {}
      );
      if (!hasDrawableData && !partial.statsDone) return;
      this._drawChart(partial.statsResult || {}, partial.events || [], t0, t1, {
    ...
    try {
      (
        ...
        .catch((err: unknown) => {
          ...
          logger.error(
            "[hass-datapoints statistics-card] statistics load failed",
            err
          );
          maybeDraw();
          finalize();
      ...
      (
        ...
        .catch((err: unknown) => {
          ...
          logger.error(
            "[hass-datapoints statistics-card] event load failed",
            err
          );
          maybeDraw();
          finalize();
  ...
  private _drawChart(
      ...
      for (const statType of this._config.stat_types as string[]) {
        ...
        if (pts.length) {
          ...
          series.push({
            ...
            unit:
              (this._hass?.states?.[statId]?.attributes
    ...
    if (!allVals.length) {
      ...
      this._chartMessage = options.loading
        ? ""
  ...
  render() {
    ...
    return html`
      ...
      >
        ...
        <div slot="legend">
          <chart-legend .series=${this._series}></chart-legend>
          ${this._eventCount > 0
            ? html`
