...
function createElement(color = "#ff9800", label = "Color") {
  const el = document.createElement("dp-color-swatch") as HTMLElement & {
    color: string;
    label: string;
  };
  el.color = color;
  ...
  describe("GIVEN a color swatch with a color", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN it displays a circle with the specified color", () => {
        const inner = el.shadowRoot!.querySelector(".swatch-inner") as HTMLElement;
