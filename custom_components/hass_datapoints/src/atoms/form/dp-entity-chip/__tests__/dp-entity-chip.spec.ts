...
function createElement(props: Record<string, unknown> = {}) {
  const el = document.createElement("dp-entity-chip") as HTMLElement & {
    type: string;
    itemId: string;
    hass: unknown;
    removable: boolean;
  };
  Object.assign(el, { type: "entity", itemId: "sensor.temperature", hass: createMockHass(), removable: false, ...props });
  document.body.appendChild(el);
  return el;
}

describe("dp-entity-chip", () => {
  ...
  describe("GIVEN a removable entity chip", () => {
    ...
    describe("WHEN the remove button is clicked", () => {
      it("THEN it fires dp-chip-remove", () => {
        ...
        expect(handler.mock.calls[0][0].detail.itemId).toBe("sensor.temperature");
      });
    });
  });
