...
function createElement(props: Record<string, unknown> = {}) {
  const el = document.createElement("dp-editor-text-field") as HTMLElement & {
    label: string;
    value: string;
    type: string;
    suffix: string;
  };
  Object.assign(el, { label: "Title", value: "My Card", type: "text", suffix: "", ...props });
  document.body.appendChild(el);
  return el;
}

describe("dp-editor-text-field", () => {
  ...
  describe("GIVEN a text field", () => {
    ...
    describe("WHEN input event fires", () => {
      it("THEN it dispatches dp-field-change with the value", () => {
        ...
        const haField = el.shadowRoot!.querySelector("ha-textfield") as HTMLElement;
