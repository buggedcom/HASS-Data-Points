...
function createElement(props: Record<string, unknown> = {}) {
  const el = document.createElement("dp-editor-entity-list") as HTMLElement & {
    entities: string[];
    hass: unknown;
    buttonLabel: string;
  };
  Object.assign(el, { entities: ["sensor.temperature"], hass: createMockHass(), buttonLabel: "Add entity", ...props });
  document.body.appendChild(el);
  return el;
}

describe("dp-editor-entity-list", () => {
  ...
  describe("GIVEN an entity list with entities", () => {
    beforeEach(async () => {
      el = createElement({ entities: ["sensor.temperature", "sensor.humidity"] });
      await el.updateComplete;
    });
    ...
    describe("WHEN a remove button is clicked", () => {
      it("THEN it dispatches dp-entity-list-change with the entity removed", () => {
        ...
        const removeBtn = el.shadowRoot!.querySelector("[data-action='remove']");
        removeBtn?.dispatchEvent(new Event("click"));
        expect(handler).toHaveBeenCalledOnce();
        expect(handler.mock.calls[0][0].detail.entities).toEqual(["sensor.humidity"]);
      });
    });
  });
