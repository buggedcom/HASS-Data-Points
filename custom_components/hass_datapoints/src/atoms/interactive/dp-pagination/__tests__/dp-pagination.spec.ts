...
function createElement(props: Record<string, unknown> = {}) {
  const el = document.createElement("dp-pagination") as HTMLElement & {
    page: number;
    totalPages: number;
    totalItems: number;
    label: string;
  };
  Object.assign(el, { page: 0, totalPages: 5, totalItems: 50, label: "records", ...props });
  document.body.appendChild(el);
  return el;
}

describe("dp-pagination", () => {
  ...
  describe("GIVEN pagination on page 3 of 5", () => {
    ...
    describe("WHEN next is clicked", () => {
      it("THEN it fires dp-page-change with page 3", () => {
        ...
        el.shadowRoot!.querySelector("[data-action='next']")!.dispatchEvent(new Event("click"));
    ...
    describe("WHEN prev is clicked", () => {
      it("THEN it fires dp-page-change with page 1", () => {
        const handler = vi.fn();
        el.addEventListener("dp-page-change", handler);
        el.shadowRoot!.querySelector("[data-action='prev']")!.dispatchEvent(new Event("click"));
        expect(handler).toHaveBeenCalledOnce();
