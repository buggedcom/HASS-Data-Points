...
import "../dp-range-handle";

function createElement(props: Record<string, unknown> = {}) {
  const el = document.createElement("dp-range-handle") as HTMLElement & {
    position: number;
    label: string;
    live: boolean;
  };
  Object.assign(el, { position: 0, label: "Start date and time", live: false, ...props });
  document.body.appendChild(el);
  return el;
}

describe("dp-range-handle", () => {
  ...
  describe("GIVEN a range handle", () => {
    ...
    describe("WHEN pointerdown fires on the button", () => {
      it("THEN it dispatches dp-handle-drag-start with pointerId and clientX", () => {
        ...
        const button = el.shadowRoot!.querySelector("button")!;
        button.dispatchEvent(new PointerEvent("pointerdown", { pointerId: 1, clientX: 120, bubbles: true }));
        expect(handler).toHaveBeenCalledOnce();
        expect(handler.mock.calls[0][0].detail).toMatchObject({ pointerId: 1, clientX: 120 });
      });
    });
    ...
    describe("WHEN ArrowLeft key is pressed", () => {
      it("THEN it dispatches dp-handle-keydown with key ArrowLeft", () => {
        ...
        expect(handler).toHaveBeenCalledOnce();
        expect(handler.mock.calls[0][0].detail).toMatchObject({ key: "ArrowLeft" });
      });
    });
    ...
    describe("WHEN PageDown key is pressed", () => {
      it("THEN it dispatches dp-handle-keydown with key PageDown", () => {
        ...
        expect(handler).toHaveBeenCalledOnce();
        expect(handler.mock.calls[0][0].detail).toMatchObject({ key: "PageDown" });
      });
    });
    ...
    describe("WHEN ArrowRight is pressed with shift", () => {
      it("THEN dp-handle-keydown includes shiftKey true", () => {
        ...
        expect(handler.mock.calls[0][0].detail).toMatchObject({ key: "ArrowRight", shiftKey: true });
      });
    });
