...
function createElement(props: Record<string, unknown> = {}) {
  const el = document.createElement("dp-annotation-chip") as HTMLElement & {
    type: string;
    itemId: string;
    icon: string;
    name: string;
...
describe("dp-annotation-chip", () => {
  ...
  describe("GIVEN an annotation chip with type, icon and name", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN it displays the name text", () => {
        expect.assertions(1);
        const text = el.shadowRoot!.querySelector(".context-chip")?.textContent?.trim();
      ...
      it("THEN the remove button has an accessible label", () => {
        expect.assertions(1);
        const removeBtn = el.shadowRoot!.querySelector(".context-chip-remove");
        expect(removeBtn?.getAttribute("aria-label")).toBe("Remove Temperature");
      });
    });
  });
  ...
  describe("GIVEN an annotation chip", () => {
    beforeEach(async () => {
      el = createElement({ type: "area", itemId: "living_room", name: "Living Room" });
      await el.updateComplete;
    });
    ...
    describe("WHEN the remove button is clicked", () => {
      it("THEN it fires dp-chip-remove with type and itemId", () => {
        ...
        el.shadowRoot!.querySelector(".context-chip-remove")!.dispatchEvent(new Event("click"));
