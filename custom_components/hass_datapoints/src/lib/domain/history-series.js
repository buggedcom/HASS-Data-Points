...
import { normalizeEntityIds } from "@/lib/domain/target-selection.js";
...
export function normalizeHistorySeriesAnalysis(analysis) {
  ...
  return {
    ...
    anomaly_methods: (() => {
      ...
      const legacy = VALID.includes(source.anomaly_method)
        ? source.anomaly_method
        : null;
      return legacy ? [legacy] : [];
    })(),
    anomaly_overlap_mode: ["all", "highlight", "only"].includes(
      source.anomaly_overlap_mode
    )
      ? source.anomaly_overlap_mode
      : "all",
    anomaly_sensitivity:
    ...
    sample_interval: (() => {
      ...
      return VALID.includes(source.sample_interval)
        ? source.sample_interval
        : "1m";
    })(),
    sample_aggregate: (() => {
      const VALID = ["mean", "min", "max", "median", "first", "last"];
      return VALID.includes(source.sample_aggregate)
...
export function buildHistorySeriesRows(entityIds, previousRows = []) {
  ...
  // When adding a new target, inherit sampling settings only when ALL existing targets
  // share the same interval (unanimous). Mixed intervals → fall back to the default "1m".
  const intervals = normalizedPrevious.map((r) => r.analysis.sample_interval);
  const allSame =
    intervals.length > 0 && intervals.every((iv) => iv === intervals[0]);
