export function getTrendWindowMs(value) {
  const windows = {
    "1h": 60 * 60 * 1000,
    "6h": 6 * 60 * 60 * 1000,
    "24h": 24 * 60 * 60 * 1000,
...
export function buildRollingAverageTrend(points, windowMs) {
  if (
...
export function buildLinearTrend(points) {
...
export function buildTrendPoints(points, method, trendWindow) {
  if (!Array.isArray(points) || points.length < 2) {
    return [];
  }
...
export function normalizeSeriesAnalysis(analysis) {
...
export function interpolateSeriesValue(points, timeMs) {
...
export function buildRateOfChangePoints(points, rateWindow) {
  if (!Array.isArray(points) || points.length < 2) {
    return [];
  }
  const ratePoints = [];
...
export function buildDeltaPoints(sourcePoints, comparisonPoints) {
  if (
...
export function buildSummaryStats(points) {
  if (!Array.isArray(points) || points.length === 0) {
    return null;
  }
  let min = Infinity;
...
export function computeHistoryAnalysis(payload) {
