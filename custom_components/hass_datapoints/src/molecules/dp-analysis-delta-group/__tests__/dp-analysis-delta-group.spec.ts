...
import "../dp-analysis-delta-group";
...
function createElement(props: {
  ...
  const el = document.createElement("dp-analysis-delta-group") as HTMLElement & {
...
describe("dp-analysis-delta-group", () => {
  ...
  describe("GIVEN canShowDeltaAnalysis=false", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN renders dp-analysis-group with disabled=true", () => {
        expect.assertions(1);
        const group = el.shadowRoot!.querySelector("dp-analysis-group") as HTMLElement & { disabled: boolean };
  ...
  describe("GIVEN canShowDeltaAnalysis=true and show_delta_analysis=true", () => {
    beforeEach(async () => {
      el = createElement({ canShowDeltaAnalysis: true, analysis: { show_delta_analysis: true } });
      await el.updateComplete;
    });
    ...
    describe("WHEN rendered", () => {
      ...
      it("THEN renders dp-analysis-group with disabled=false", () => {
        expect.assertions(1);
        const group = el.shadowRoot!.querySelector("dp-analysis-group") as HTMLElement & { disabled: boolean };
  ...
  describe("GIVEN a rendered delta group", () => {
    beforeEach(async () => {
      el = createElement({ canShowDeltaAnalysis: true, analysis: { show_delta_analysis: false } });
      await el.updateComplete;
    });
