import type { ComponentTranslations } from "@/lib/i18n/types";

export const translations: ComponentTranslations = {
  "Show delta vs selected date window":
    "Delta gegenüber dem ausgewählten Datumsfenster anzeigen",
  "Select a date window tab to enable delta analysis.":
    "Wählen Sie einen Tab für ein Datumsfenster, um die Delta-Analyse zu aktivieren.",
  "Show delta in tooltip": "Delta im Tooltip anzeigen",
  "Show delta lines": "Delta-Linien anzeigen",
};
