...
function createElement(props: { analysis?: Partial<NormalizedAnalysis>; entityId?: string } = {}) {
  const el = document.createElement("dp-analysis-rate-group") as HTMLElement & {
...
describe("dp-analysis-rate-group", () => {
  let el: ReturnType<typeof createElement>;

  afterEach(() => el?.remove());

  describe("GIVEN show_rate_of_change=false", () => {
    beforeEach(async () => {
      el = createElement({ analysis: { show_rate_of_change: false } });
  ...
  describe("GIVEN show_rate_of_change=true", () => {
    beforeEach(async () => {
      el = createElement({ analysis: { show_rate_of_change: true } });
      await el.updateComplete;
    });
  ...
  describe("GIVEN a rendered rate group", () => {
    beforeEach(async () => {
      el = createElement({ analysis: { show_rate_of_change: false } });
      await el.updateComplete;
    });
    ...
    describe("WHEN dp-group-change fires with checked=true", () => {
      it("THEN dispatches dp-group-analysis-change with key=show_rate_of_change and value=true", () => {
        expect.assertions(3);
        const handler = vi.fn();
        ...
        expect(handler.mock.calls[0][0].detail.key).toBe("show_rate_of_change");
