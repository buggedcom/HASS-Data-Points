...
import "../dp-analysis-summary-group";
...
function createElement(props: { analysis?: Partial<NormalizedAnalysis>; entityId?: string } = {}) {
  const el = document.createElement("dp-analysis-summary-group") as HTMLElement & {
...
describe("dp-analysis-summary-group", () => {
  ...
  describe("GIVEN show_summary_stats=false", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN dp-analysis-group has checked=false", () => {
        expect.assertions(1);
        const group = el.shadowRoot!.querySelector("dp-analysis-group") as HTMLElement & { checked: boolean };
        expect(group.checked).toBe(false);
      });

      it("THEN dp-analysis-group does not render its group body (slot content hidden)", () => {
        expect.assertions(1);
  ...
  describe("GIVEN show_summary_stats=true", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN dp-analysis-group has checked=true", () => {
        expect.assertions(1);
        const group = el.shadowRoot!.querySelector("dp-analysis-group") as HTMLElement & { checked: boolean };
      ...
      it("THEN the shading checkbox is visible and unchecked", () => {
        expect.assertions(2);
        const shadingCheckbox = el.shadowRoot!.querySelector("label.option input[type='checkbox']") as HTMLInputElement | null;
  ...
  describe("GIVEN show_summary_stats=true and show_summary_stats_shading=true", () => {
    beforeEach(async () => {
      el = createElement({ analysis: { show_summary_stats: true, show_summary_stats_shading: true } });
      await el.updateComplete;
    });
  ...
  describe("GIVEN a rendered summary group", () => {
    ...
    describe("WHEN dp-group-change fires with checked=false", () => {
      it("THEN dispatches dp-group-analysis-change with key=show_summary_stats value=false", () => {
        ...
        el.addEventListener("dp-group-analysis-change", handler);
        const group = el.shadowRoot!.querySelector("dp-analysis-group")!;
        group.dispatchEvent(new CustomEvent("dp-group-change", { detail: { checked: false }, bubbles: true, composed: true }));
        expect(handler).toHaveBeenCalledOnce();
        expect(handler.mock.calls[0][0].detail.key).toBe("show_summary_stats");
  ...
  describe("GIVEN show_summary_stats=true and show_summary_stats_shading=false", () => {
    beforeEach(async () => {
      el = createElement({ analysis: { show_summary_stats: true, show_summary_stats_shading: false } });
      await el.updateComplete;
    });
    ...
    describe("WHEN the shading checkbox is changed to checked", () => {
      it("THEN dispatches dp-group-analysis-change with key=show_summary_stats_shading value=true", () => {
        ...
        const handler = vi.fn();
        el.addEventListener("dp-group-analysis-change", handler);
        const shadingCheckbox = el.shadowRoot!.querySelector("label.option input[type='checkbox']") as HTMLInputElement;
        shadingCheckbox.checked = true;
        shadingCheckbox.dispatchEvent(new Event("change", { bubbles: true, composed: true }));
        expect(handler).toHaveBeenCalledOnce();
        expect(handler.mock.calls[0][0].detail.key).toBe("show_summary_stats_shading");
        expect(handler.mock.calls[0][0].detail.value).toBe(true);
      });
    });
  });
  ...
  describe("GIVEN entityId=sensor.living_room_temp", () => {
    beforeEach(async () => {
      el = createElement({ entityId: "sensor.living_room_temp", analysis: { show_summary_stats: false } });
      await el.updateComplete;
    });
    ...
    describe("WHEN dp-group-change fires", () => {
      it("THEN the dispatched event detail includes the correct entityId", () => {
        ...
        const group = el.shadowRoot!.querySelector("dp-analysis-group")!;
        group.dispatchEvent(new CustomEvent("dp-group-change", { detail: { checked: true }, bubbles: true, composed: true }));
        expect(handler.mock.calls[0][0].detail.entityId).toBe("sensor.living_room_temp");
      });
    });
  });
});
