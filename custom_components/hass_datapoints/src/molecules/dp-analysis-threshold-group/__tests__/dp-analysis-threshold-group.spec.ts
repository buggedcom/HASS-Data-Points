...
import "../dp-analysis-threshold-group";
...
function createElement(props: { analysis?: Partial<NormalizedAnalysis>; entityId?: string; unit?: string } = {}) {
  const el = document.createElement("dp-analysis-threshold-group") as HTMLElement & {
...
describe("dp-analysis-threshold-group", () => {
  let el: ReturnType<typeof createElement>;

  afterEach(() => el?.remove());

  describe("GIVEN show_threshold_analysis=false", () => {
    beforeEach(async () => {
      el = createElement({ analysis: { show_threshold_analysis: false } });
  ...
  describe("GIVEN show_threshold_analysis=true", () => {
    beforeEach(async () => {
      el = createElement({ analysis: { show_threshold_analysis: true } });
      await el.updateComplete;
    });
  ...
  describe("GIVEN a rendered threshold group", () => {
    beforeEach(async () => {
      el = createElement({ analysis: { show_threshold_analysis: false } });
      await el.updateComplete;
    });
    ...
    describe("WHEN dp-group-change fires with checked=true", () => {
      it("THEN dispatches dp-group-analysis-change with key=show_threshold_analysis and value=true", () => {
        expect.assertions(3);
        const handler = vi.fn();
        ...
        expect(handler.mock.calls[0][0].detail.key).toBe("show_threshold_analysis");
