...
const COMPARISON_TABS: TabItem[] = [
  ...
  { id: "window-2", label: "Summer 2024", detail: "1 Jun – 31 Aug 2024", active: false, editable: true },
];

/**
 * `dp-comparison-tab-rail` renders the full comparison tab bar, consisting of a
 * scrollable row of `dp-comparison-tab` elements and an "Add date window" button.
 *
...
export default {
  ...
  component: "dp-comparison-tab-rail",
  parameters: {
    actions: {
      handles: ["dp-tab-activate", "dp-tab-hover", "dp-tab-leave", "dp-tab-edit", "dp-tab-delete", "dp-tab-add"],
    },
  },
  argTypes: {
    tabs: {
    ...
    loadingIds: {
      control: "object",
      description: "Array of tab IDs currently loading data. Matching tabs show a spinner.",
    },
    hoveredId: {
      control: "text",
      description: "ID of the tab currently being previewed (hovered). Sets `previewing` on that tab.",
    },
    overflowing: {
      control: "boolean",
      description: "When true, the rail is overflowing and the Add button collapses to an icon.",
    },
  },
  args: {
...
export const EmitsTabAdd = {
  play: async ({ canvasElement }: { canvasElement: HTMLElement }) => {
    const host = canvasElement.querySelector("dp-comparison-tab-rail") as HTMLElement;
    const handler = fn();
    host.addEventListener("dp-tab-add", handler);
    const canvas = within(host.shadowRoot as unknown as HTMLElement);
    await userEvent.click(canvas.getByRole("button", { name: /add date window/i }));
