...
import "../dp-date-window-dialog";

function createElement(props: Record<string, unknown> = {}) {
  const el = document.createElement("dp-date-window-dialog") as HTMLElement & {
    open: boolean;
    heading: string;
    name: string;
    startValue: string;
...
describe("dp-date-window-dialog", () => {
  ...
  describe("GIVEN default props with open=true", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN renders the name field", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.querySelector("#date-window-name")).not.toBeNull();
      });

      it("THEN renders the start datetime input", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.querySelector("#date-window-start")).not.toBeNull();
      });
      ...
      it("THEN renders the cancel button", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.querySelector(".date-window-dialog-cancel")).not.toBeNull();
      });

      it("THEN renders the submit button with the submitLabel", () => {
        expect.assertions(1);
        const submit = el.shadowRoot!.querySelector(".date-window-dialog-submit");
        expect(submit?.textContent?.trim()).toBe("Create date window");
      });

      it("THEN does not render the delete button by default", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.querySelector(".date-window-dialog-delete")).toBeNull();
      });

      it("THEN does not render shortcut buttons by default", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.querySelector(".date-window-dialog-shortcuts")).toBeNull();
      });
    });
  });
  ...
  describe("GIVEN showDelete=true", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN renders the delete button", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.querySelector(".date-window-dialog-delete")).not.toBeNull();
      });
    });
  });
  ...
  describe("GIVEN showShortcuts=true", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN renders the shortcuts container", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.querySelector(".date-window-dialog-shortcuts")).not.toBeNull();
      });
    });
  });
  ...
  describe("GIVEN name='Heating season' and submitLabel='Save date window'", () => {
    beforeEach(async () => {
      el = createElement({ name: "Heating season", submitLabel: "Save date window" });
      await el.updateComplete;
    });
  ...
  describe("GIVEN the component is open and rendered", () => {
    ...
    describe("WHEN the cancel button is clicked", () => {
      it("THEN fires dp-window-close", () => {
        ...
        el.shadowRoot!.querySelector<HTMLElement>(".date-window-dialog-cancel")!.click();
    ...
    describe("WHEN the submit button is clicked", () => {
      it("THEN fires dp-window-submit", () => {
        ...
        el.shadowRoot!.querySelector<HTMLElement>(".date-window-dialog-submit")!.click();
        expect(handler).toHaveBeenCalledOnce();
      });
      ...
      it("THEN dp-window-submit detail includes name, start, end fields", () => {
        ...
        el.addEventListener("dp-window-submit", handler);
        el.shadowRoot!.querySelector<HTMLElement>(".date-window-dialog-submit")!.click();
        const detail = handler.mock.calls[0][0].detail as { name: string; start: string; end: string };
  ...
  describe("GIVEN showDelete=true and rendered", () => {
    ...
    describe("WHEN the delete button is clicked", () => {
      it("THEN fires dp-window-delete", () => {
        ...
        el.shadowRoot!.querySelector<HTMLElement>(".date-window-dialog-delete")!.click();
        expect(handler).toHaveBeenCalledOnce();
      });
    });
  });
  ...
  describe("GIVEN showShortcuts=true and rendered", () => {
    ...
    describe("WHEN the previous range shortcut is clicked", () => {
      it("THEN fires dp-window-shortcut with direction=-1", () => {
        ...
        const shortcuts = el.shadowRoot!.querySelector(".date-window-dialog-shortcuts");
