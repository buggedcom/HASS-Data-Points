...
export default {
  ...
  component: "dp-date-window-dialog",
  parameters: {
    actions: {
      handles: ["dp-window-close", "dp-window-submit", "dp-window-delete", "dp-window-shortcut", "dp-window-date-change"],
    },
  },
  argTypes: {
    open: {
    ...
    showDelete: {
      control: "boolean",
      description: "Whether the Delete button is shown (true when editing an existing window).",
    },
    showShortcuts: {
      control: "boolean",
      description: "Whether the 'Use previous/next range' shortcut buttons are shown.",
    },
    submitLabel: {
      control: "text",
      description: "Label for the submit button. E.g. \"Create date window\" or \"Save date window\".",
    },
  },
  args: {
    open: true,
...
export const EmitsSubmit = {
  ...
  play: async ({ canvasElement }: { canvasElement: HTMLElement }) => {
    const host = canvasElement.querySelector("dp-date-window-dialog") as HTMLElement;
    const handler = fn();
    ...
    const detail = (handler.mock.calls[0][0] as CustomEvent).detail as { name: string; start: string; end: string };
