...
import "../dp-editor-base";

describe("dp-editor-base", () => {
  let el: HTMLElement & { _config: Record<string, unknown>; hass: unknown; setConfig: (c: unknown) => void; _set: (k: string, v: unknown) => void };

  afterEach(() => el?.remove());

  describe("GIVEN a base editor", () => {
    beforeEach(async () => {
      el = document.createElement("dp-editor-base") as any;
    ...
    describe("WHEN _set is called with null", () => {
      it("THEN it fires config-changed with the key removed", () => {
        ...
        expect(handler).toHaveBeenCalledOnce();
        expect(handler.mock.calls[0][0].detail.config.hours_to_show).toBeUndefined();
      });
    });
  });
});
