...
function createElement(props: Record<string, unknown> = {}) {
  const el = document.createElement("dp-comparison-tab") as HTMLElement & {
    tabId: string;
    label: string;
    detail: string;
    active: boolean;
  ...
  describe("GIVEN default props", () => {
    ...
    describe("WHEN rendered", () => {
      ...
      it("THEN renders the trigger button", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.querySelector(".chart-tab-trigger")).not.toBeNull();
      });
  ...
  describe("GIVEN active=true", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN chart-tab has the active class", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.querySelector(".chart-tab")?.classList.contains("active")).toBe(true);
      });
  ...
  describe("GIVEN previewing=true", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN chart-tab has the previewing class", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.querySelector(".chart-tab")?.classList.contains("previewing")).toBe(true);
      });
    });
  });
  ...
  describe("GIVEN loading=true", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN chart-tab has the loading class", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.querySelector(".chart-tab")?.classList.contains("loading")).toBe(true);
      });

      it("THEN the spinner is rendered", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.querySelector(".chart-tab-spinner")).not.toBeNull();
      });
    });
  });
  ...
  describe("GIVEN editable=true", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN renders the chart-tab-actions container", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.querySelector(".chart-tab-actions")).not.toBeNull();
      });

      it("THEN renders the edit button", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.querySelector(".chart-tab-action.edit")).not.toBeNull();
      });

      it("THEN renders the delete button", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.querySelector(".chart-tab-action.delete")).not.toBeNull();
      });
    });
  });
  ...
  describe("GIVEN the component is rendered", () => {
    ...
    describe("WHEN the trigger button is clicked", () => {
      it("THEN fires dp-tab-activate with the tabId", () => {
        ...
        el.shadowRoot!.querySelector<HTMLButtonElement>(".chart-tab-trigger")!.click();
    ...
    describe("WHEN mouseenter fires on the chart-tab", () => {
      it("THEN fires dp-tab-hover with the tabId", () => {
        ...
        el.shadowRoot!.querySelector(".chart-tab")!.dispatchEvent(new MouseEvent("mouseenter", { bubbles: true }));
        expect(handler).toHaveBeenCalledOnce();
        expect(handler.mock.calls[0][0].detail.tabId).toBe("win-abc");
      });
    });
    ...
    describe("WHEN mouseleave fires on the chart-tab", () => {
      it("THEN fires dp-tab-leave with the tabId", () => {
        ...
        el.shadowRoot!.querySelector(".chart-tab")!.dispatchEvent(new MouseEvent("mouseleave", { bubbles: true }));
        expect(handler).toHaveBeenCalledOnce();
        expect(handler.mock.calls[0][0].detail.tabId).toBe("win-abc");
      });
    });
  ...
  describe("GIVEN editable=true and rendered", () => {
    ...
    describe("WHEN the edit button is clicked", () => {
      it("THEN fires dp-tab-edit with the tabId", () => {
        ...
        el.shadowRoot!.querySelector<HTMLButtonElement>(".chart-tab-action.edit")!.click();
        expect(handler).toHaveBeenCalledOnce();
    ...
    describe("WHEN the delete button is clicked", () => {
      it("THEN fires dp-tab-delete with the tabId", () => {
        ...
        el.shadowRoot!.querySelector<HTMLButtonElement>(".chart-tab-action.delete")!.click();
        expect(handler).toHaveBeenCalledOnce();
        expect(handler.mock.calls[0][0].detail.tabId).toBe("win-xyz");
      });
    });
