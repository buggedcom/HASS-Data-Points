...
function createElement(props: Record<string, unknown> = {}) {
  const el = document.createElement("dp-panel-timeline") as HTMLElement & {
    startTime: Date | null;
    endTime: Date | null;
...
describe("dp-panel-timeline", () => {
  ...
  describe("GIVEN default props", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN renders a dp-range-timeline", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.querySelector("dp-range-timeline")).not.toBeNull();
      });

      it("THEN renders the hover preview overlay", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.getElementById("range-hover-preview")).not.toBeNull();
      });

      it("THEN renders the comparison preview overlay", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.getElementById("range-comparison-preview")).not.toBeNull();
      });

      it("THEN renders the zoom highlight overlay", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.getElementById("range-zoom-highlight")).not.toBeNull();
      });

      it("THEN renders the zoom window highlight overlay", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.getElementById("range-zoom-window-highlight")).not.toBeNull();
      });

      it("THEN renders the chart hover line overlay", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.getElementById("range-chart-hover-line")).not.toBeNull();
      });

      it("THEN renders the chart hover window line overlay", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.getElementById("range-chart-hover-window-line")).not.toBeNull();
      });

      it("THEN renders the event layer", () => {
        expect.assertions(1);
        expect(el.shadowRoot!.getElementById("range-event-layer")).not.toBeNull();
      });

      it("THEN no overlay has the visible class by default", () => {
        expect.assertions(4);
        const sr = el.shadowRoot!;
        expect(sr.getElementById("range-hover-preview")!.classList.contains("visible")).toBe(false);
        expect(sr.getElementById("range-comparison-preview")!.classList.contains("visible")).toBe(false);
        expect(sr.getElementById("range-zoom-highlight")!.classList.contains("visible")).toBe(false);
        expect(sr.getElementById("range-chart-hover-line")!.classList.contains("visible")).toBe(false);
      });
    });
  });
  ...
  describe("GIVEN isLiveEdge=true forwarded to the inner atom", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN the inner dp-range-timeline has isLiveEdge=true", async () => {
        expect.assertions(1);
        const atom = el.shadowRoot!.querySelector("dp-range-timeline") as HTMLElement & {
          isLiveEdge: boolean;
          updateComplete: Promise<boolean>;
        };
  ...
  describe("GIVEN rangeBounds forwarded to the inner atom", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN the inner dp-range-timeline receives the rangeBounds object", async () => {
        expect.assertions(1);
        const atom = el.shadowRoot!.querySelector("dp-range-timeline") as HTMLElement & {
          rangeBounds: RangeBounds | null;
          updateComplete: Promise<boolean>;
  ...
  describe("GIVEN comparisonPreview set with rangeBounds", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN the comparison preview has the visible class", () => {
        expect.assertions(1);
        const preview = el.shadowRoot!.getElementById("range-comparison-preview")!;
        expect(preview.classList.contains("visible")).toBe(true);
      });
    });
  });
  ...
  describe("GIVEN zoomRange set with rangeBounds", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN the zoom highlight has the visible class", () => {
        expect.assertions(1);
        const highlight = el.shadowRoot!.getElementById("range-zoom-highlight")!;
