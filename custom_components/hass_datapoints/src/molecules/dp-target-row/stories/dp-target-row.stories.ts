...
async function getEl(canvasElement: HTMLElement): Promise<DpTargetRowEl> {
  const el = canvasElement.querySelector("dp-target-row") as DpTargetRowEl;
...
/**
 * `dp-target-row` is a single entity row in the history panel sidebar.
...
export default {
  ...
  argTypes: {
    ...
    visible: {
      control: "boolean",
      description: "Whether the series is shown on the chart — controls the toggle state and `is-hidden` opacity",
    },
    canShowDeltaAnalysis: {
      control: "boolean",
      description: "Enable delta analysis options (requires a comparison date window to be active)",
    },
    stateObj: {
      control: "object",
      description: "HA entity state object from `hass.states`. Provides the display name (`attributes.friendly_name`), unit (`attributes.unit_of_measurement`), and icon for `ha-state-icon`.",
    },
    index: {
      control: "number",
      description: "Zero-based position of this row in the list — included in `dp-row-remove` and `dp-row-color-change` events",
    },
    analysis: {
      control: "object",
      description: "Normalized analysis config object with all analysis settings as a flat record. The `expanded` field controls whether the analysis panel is open.",
    },
    comparisonWindows: {
      control: "object",
      description: "Array of `{ id, label? }` date windows available for comparison / delta analysis",
    },
  },
...
export const MultipleRows = {
    ...
    <div style="max-width: 600px; display: grid; gap: 10px;">
      <dp-target-row
        .stateObj=${{
          entity_id: "sensor.living_room_temperature",
          state: "21.5",
          attributes: { friendly_name: "Living Room Temperature", unit_of_measurement: "°C", icon: "mdi:thermometer" },
          last_changed: "",
          last_updated: "",
          context: null,
        }}
      ...
      <dp-target-row
        .stateObj=${{
          entity_id: "sensor.outdoor_humidity",
          state: "72",
          attributes: { friendly_name: "Outdoor Humidity", unit_of_measurement: "%", icon: "mdi:water-percent" },
          last_changed: "",
          last_updated: "",
          context: null,
        }}
      ...
      <dp-target-row
        .stateObj=${{
          entity_id: "sensor.energy_consumption",
          state: "12.4",
          attributes: { friendly_name: "Energy Consumption", unit_of_measurement: "kWh", icon: "mdi:lightning-bolt" },
          last_changed: "",
          last_updated: "",
          context: null,
        }}
  ...
  `,
  play: async ({ canvasElement }: { canvasElement: HTMLElement }) => {
    const rows = canvasElement.querySelectorAll("dp-target-row");
