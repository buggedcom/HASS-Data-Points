...
const SAMPLE_ROWS: RowConfig[] = [
  { entity_id: "sensor.temperature", color: "#03a9f4", visible: true, analysis: BLANK_ANALYSIS },
  { entity_id: "sensor.humidity", color: "#e040fb", visible: false, analysis: BLANK_ANALYSIS },
  { entity_id: "sensor.pressure", color: "#69f0ae", visible: true, analysis: BLANK_ANALYSIS },
];
...
const SAMPLE_STATES: Record<string, Record<string, unknown>> = {
  ...
  "sensor.humidity": {
    ...
    state: "55",
    attributes: { friendly_name: "Humidity", unit_of_measurement: "%", icon: "mdi:water-percent" },
    last_changed: "2024-01-01T00:00:00.000Z",
    last_updated: "2024-01-01T00:00:00.000Z",
  ...
  "sensor.pressure": {
    ...
    state: "1013",
    attributes: { friendly_name: "Pressure", unit_of_measurement: "hPa", icon: "mdi:gauge" },
    last_changed: "2024-01-01T00:00:00.000Z",
    last_updated: "2024-01-01T00:00:00.000Z",
...
function createElement(
  ...
  const el = document.createElement("dp-target-row-list") as HTMLElement & {
    rows: RowConfig[];
    states: Record<string, Record<string, unknown>>;
    canShowDeltaAnalysis: boolean;
    comparisonWindows: Array<{ id: string; label?: string }>;
...
describe("dp-target-row-list", () => {
  ...
  describe("GIVEN a list with rows", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN renders one dp-target-row per row", () => {
        expect.assertions(1);
      ...
      it("THEN passes stateObj to the first row", async () => {
        ...
        expect((firstRow.stateObj?.attributes as Record<string, unknown>)?.friendly_name).toBe("Temperature");
      });
      ...
      it("THEN marks each row with data-row-index", () => {
        expect.assertions(3);
        const rows = el.shadowRoot!.querySelectorAll("dp-target-row[data-row-index]");
  ...
  describe("GIVEN an empty list", () => {
    ...
    describe("WHEN rendered", () => {
      ...
      it("THEN does not render any dp-target-row elements", () => {
        expect.assertions(1);
        const rows = el.shadowRoot!.querySelectorAll("dp-target-row");
  ...
  describe("GIVEN a list with an expanded entity", () => {
    beforeEach(async () => {
      el = createElement({
        rows: [
          SAMPLE_ROWS[0],
          { ...SAMPLE_ROWS[1], analysis: { ...BLANK_ANALYSIS, expanded: true } },
          SAMPLE_ROWS[2],
        ],
      });
      await el.updateComplete;
    ...
    describe("WHEN rendered", () => {
      it("THEN passes analysis.expanded=true to the matching row and false to others", async () => {
        expect.assertions(2);
        const rows = el.shadowRoot!.querySelectorAll("dp-target-row") as NodeListOf<
          HTMLElement & { analysis: NormalizedAnalysis; updateComplete: Promise<boolean> }
        >;
        await rows[1].updateComplete;
    ...
    describe("WHEN a reorder drop fires", () => {
      it("THEN dispatches dp-rows-reorder with updated rows array", () => {
        ...
        rows[0].dispatchEvent(
          new DragEvent("dragstart", { dataTransfer: new DataTransfer(), bubbles: false, cancelable: true }),
        );
