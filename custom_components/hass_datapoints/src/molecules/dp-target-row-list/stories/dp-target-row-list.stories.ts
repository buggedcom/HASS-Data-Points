...
const SAMPLE_STATES: Record<string, Record<string, unknown>> = {
  "sensor.living_room_temperature": {
    entity_id: "sensor.living_room_temperature",
    state: "21.5",
    attributes: { friendly_name: "Living Room", unit_of_measurement: "°C", icon: "mdi:thermometer", device_class: "temperature" },
  ...
  "sensor.bedroom_temperature": {
    entity_id: "sensor.bedroom_temperature",
    state: "19.2",
    attributes: { friendly_name: "Bedroom", unit_of_measurement: "°C", icon: "mdi:thermometer", device_class: "temperature" },
    last_changed: "2024-01-01T00:00:00.000Z",
    last_updated: "2024-01-01T00:00:00.000Z",
  ...
  "sensor.outdoor_temperature": {
    entity_id: "sensor.outdoor_temperature",
    state: "8.0",
    attributes: { friendly_name: "Outdoor", unit_of_measurement: "°C", icon: "mdi:thermometer-auto", device_class: "temperature" },
    last_changed: "2024-01-01T00:00:00.000Z",
    last_updated: "2024-01-01T00:00:00.000Z",
...
async function getEl(canvasElement: HTMLElement): Promise<ListEl> {
  const el = canvasElement.querySelector("dp-target-row-list") as ListEl;
...
/**
 * `dp-target-row-list` renders an ordered list of `dp-target-row` elements
...
export default {
  ...
  argTypes: {
    ...
    states: {
      control: "object",
      description: "Map of entity_id → HA entity state object. Passed to `ha-state-icon` inside each colour swatch.",
    },
    canShowDeltaAnalysis: {
      control: "boolean",
      description: "Whether at least one comparison window exists, enabling the delta analysis option.",
    },
    comparisonWindows: {
      control: "object",
      description: "Array of date-window comparison objects available for delta analysis.",
    },
  },
  args: {
    rows: SAMPLE_ROWS,
...
export const Default = {
  play: async ({ canvasElement }: { canvasElement: HTMLElement }) => {
    ...
    const rows = el.shadowRoot.querySelectorAll("dp-target-row");
...
export const WithExpandedAnalysis = {
  ...
  play: async ({ canvasElement }: { canvasElement: HTMLElement }) => {
    const el = await getEl(canvasElement);
    const firstRow = el.shadowRoot.querySelector("dp-target-row") as HTMLElement & {
      updateComplete: Promise<boolean>;
    };
...
export const SingleRow = {
  args: {
    rows: [SAMPLE_ROWS[0]],
    states: { "sensor.living_room_temperature": SAMPLE_STATES["sensor.living_room_temperature"] },
  },
  play: async ({ canvasElement }: { canvasElement: HTMLElement }) => {
    const el = await getEl(canvasElement);
    const rows = el.shadowRoot.querySelectorAll("dp-target-row");
...
export const ManyRows = {
  args: {
    rows: [
      ...SAMPLE_ROWS,
      { entity_id: "sensor.kitchen_temperature", color: "#ff7043", visible: true, analysis: BLANK_ANALYSIS },
      { entity_id: "sensor.garage_temperature", color: "#ffd740", visible: true, analysis: BLANK_ANALYSIS },
    ],
    states: {
      ...SAMPLE_STATES,
      "sensor.kitchen_temperature": {
        entity_id: "sensor.kitchen_temperature", state: "22.1",
        attributes: { friendly_name: "Kitchen", unit_of_measurement: "°C", icon: "mdi:thermometer", device_class: "temperature" },
        last_changed: "2024-01-01T00:00:00.000Z", last_updated: "2024-01-01T00:00:00.000Z",
        context: { id: "abc4", parent_id: null, user_id: null },
      },
      "sensor.garage_temperature": {
        entity_id: "sensor.garage_temperature", state: "5.3",
        attributes: { friendly_name: "Garage", unit_of_measurement: "°C", icon: "mdi:thermometer", device_class: "temperature" },
        last_changed: "2024-01-01T00:00:00.000Z", last_updated: "2024-01-01T00:00:00.000Z",
...
export const EmitsRemove = {
  ...
  play: async ({ canvasElement }: { canvasElement: HTMLElement }) => {
    ...
    const rows = el.shadowRoot.querySelectorAll("dp-target-row") as NodeListOf<
...
export const EmitsReorder = {
  play: async ({ canvasElement }: { canvasElement: HTMLElement }) => {
    ...
    // and composedPath() will include the dp-target-row element so _rowFromEvent finds it.
