...
function createElement(props: Record<string, unknown> = {}) {
  const el = document.createElement("dp-sidebar-datapoints-section") as HTMLElement & {
...
describe("dp-sidebar-datapoints-section", () => {
  ...
  describe("GIVEN datapointScope is linked", () => {
    ...
    describe("WHEN rendered", () => {
      ...
      it("THEN renders a dp-radio-group inside the section slot", () => {
        expect.assertions(1);
  ...
  describe("GIVEN the component is rendered", () => {
    ...
    describe("WHEN the radio group emits dp-radio-change with value=all", () => {
      it("THEN dispatches dp-scope-change with value=all", () => {
        ...
        el.shadowRoot!.querySelector("dp-radio-group")!.dispatchEvent(
          new CustomEvent("dp-radio-change", { detail: { value: "all" }, bubbles: true, composed: true }),
        );
