...
import "../dp-chip-group";

function createElement(props: Record<string, unknown> = {}) {
  const el = document.createElement("dp-chip-group") as HTMLElement & {
...
describe("dp-chip-group", () => {
  ...
  describe("GIVEN a chip group with two items", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN it renders two dp-entity-chip elements", () => {
        const chips = el.shadowRoot!.querySelectorAll("dp-entity-chip");
  ...
  describe("GIVEN a chip group with removable chips", () => {
    ...
    describe("WHEN a chip fires dp-chip-remove", () => {
      it("THEN it re-dispatches dp-chips-change with the item removed", () => {
        ...
        const chip = el.shadowRoot!.querySelector("dp-entity-chip")!;
        chip.dispatchEvent(
        ...
        expect(handler.mock.calls[0][0].detail.items).toEqual([{ type: "device", id: "device_1" }]);
      });
    });
  });
