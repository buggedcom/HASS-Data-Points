...
export class HassRecordsDevToolCard extends HTMLElement {
  constructor() {
    ...
    // Map<windowId, { label, startDt, hours, changes, selected: Set<idx> }>
  ...
  _updateHassOnChildren() {
    ...
    if (!ep) return;
    // Suppress value-changed events fired by selector's async re-render when hass changes
  ...
  _syncRemoveButtons() {
    ...
    rows.forEach(r => { r.querySelector(".remove-window-btn").disabled = rows.length <= 1; });
  }
  ...
  _readWindowConfigs() {
    return [...this.shadowRoot.querySelectorAll(".window-row")].map((row, idx) => {
      ...
      const startDt = row.querySelector(".w-start").value; // "YYYY-MM-DDTHH:MM" local or ""
  ...
  async _analyzeHistory() {
    ...
    try {
      ...
      await Promise.all(windowConfigs.map(async (w) => {
        const start = w.startDt ? new Date(w.startDt) : new Date(now.getTime() - w.hours * 3600_000);
        const end   = w.startDt ? new Date(start.getTime() + w.hours * 3600_000) : now;
  ...
  _detectChanges(histResult) {
    // HA history/history_during_period compact format:
    //   s.s  = state string   (NOT s.state)
    //   s.lc = last_changed Unix seconds
    //   s.lu = last_updated Unix seconds
    //   s.a  = attributes dict
    const changes = [];
    ...
    for (const [entityId, states] of Object.entries(histResult)) {
      ...
      for (let i = 0; i < states.length; i++) {
        ...
        if (prev && prevVal === cur) {
          if (domain !== "climate") continue; // attribute-only update
        }
  ...
  _renderResults() {
    ...
    container.innerHTML = orderedIds.map(wid => {
      ...
      const rangeLabel = startDt
        ? `from ${new Date(startDt).toLocaleString([], { dateStyle: "short", timeStyle: "short" })} \u00b7 ${hours}h`
        : `most recent ${hours}h`;
    ...
    container.querySelectorAll(".window-result-header").forEach(header => {
      header.addEventListener("click", e => {
        if (e.target.closest(".window-result-links")) return; // don't collapse on link clicks
  ...
  async _recordSelected() {
    ...
    for (const item of allItems) {
      try {
        await this._hass.callService(DOMAIN, "record", {
  ...
  async _deleteAllDev() {
    ...
    if (count === 0) { this._showStatus("delete-status", "err", "No dev datapoints to delete."); return; }
    ...
    btn.disabled = true;
    try {
      const result = await this._hass.connection.sendMessagePromise({ type: `${DOMAIN}/events/delete_dev` });
  ...
  async _refreshDevCount() {
    try {
      const result = await this._hass.connection.sendMessagePromise({ type: `${DOMAIN}/events` });
