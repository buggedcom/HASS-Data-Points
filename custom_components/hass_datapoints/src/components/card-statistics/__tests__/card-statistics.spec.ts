...
vi.mock("@/lib/shared.js", async (importOriginal) => {
  ...
  return {
    ...
    attachLineChartRangeZoom: vi.fn().mockReturnValue(() => {}),
...
describe("card-statistics", () => {
  ...
  describe("GIVEN a card with default config", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN it uses a dp-chart-shell wrapper", () => {
        expect(el.shadowRoot.querySelector("dp-chart-shell")).toBeTruthy();
      });

      it("THEN it passes the title to dp-chart-shell", () => {
        const shell = el.shadowRoot.querySelector("dp-chart-shell");
      ...
      it("THEN it has a dp-chart-legend", () => {
        expect(el.shadowRoot.querySelector("dp-chart-legend")).toBeTruthy();
      });
    });
  });
  ...
  describe("GIVEN a card without a title", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN dp-chart-shell has an empty card title", () => {
        const shell = el.shadowRoot.querySelector("dp-chart-shell");
  ...
  describe("GIVEN a card where statistics are loading", () => {
    beforeEach(async () => {
      ...
      vi.mocked(fetchStatisticsDuringPeriod).mockReturnValue(new Promise(() => {}));
    ...
    describe("WHEN the data is still loading", () => {
      it("THEN dp-chart-shell shows the loading indicator", () => {
        const shell = el.shadowRoot.querySelector("dp-chart-shell");
  ...
  describe("GIVEN a card where statistics fail to load", () => {
    beforeEach(async () => {
      vi.mocked(fetchStatisticsDuringPeriod).mockRejectedValue(new Error("Network error"));
    ...
    describe("WHEN the load fails", () => {
      it("THEN dp-chart-shell shows an error message", () => {
        const shell = el.shadowRoot.querySelector("dp-chart-shell");
  ...
  describe("GIVEN setConfig with missing entity", () => {
    describe("WHEN setConfig is called without entity or entities", () => {
      it("THEN it throws an error", () => {
        const el2 = document.createElement("hass-datapoints-statistics-card") as any;
  ...
  describe("GIVEN the static config methods", () => {
    ...
    describe("WHEN getConfigElement is called", () => {
      it("THEN it returns the editor element tag", () => {
        ...
        expect(editorEl.tagName.toLowerCase()).toBe("hass-datapoints-statistics-card-editor");
      });
    });
  });
});
