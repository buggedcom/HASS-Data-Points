...
import { HassRecordsActionCard } from "../card-action.ts";
...
describe("card-action", () => {
  ...
  describe("GIVEN a card with default config", () => {
    ...
    describe("WHEN rendered", () => {
      ...
      it("THEN it has a color swatch", () => {
        expect(el.shadowRoot.querySelector("dp-color-swatch")).toBeTruthy();
      });

      it("THEN it has an icon picker", () => {
        expect(el.shadowRoot.querySelector("ha-icon-picker#icon-picker")).toBeTruthy();
      });
    });
  });
  ...
  describe("GIVEN a card with an entity in config", () => {
    ...
    describe("WHEN rendered", () => {
      it("THEN it shows a dp-chip-group for config targets", () => {
        expect(el.shadowRoot.querySelector("dp-chip-group")).toBeTruthy();
      });
    });
  });
  ...
  describe("GIVEN a card with show_config_targets: false", () => {
    beforeEach(async () => {
      el = createCard({ entity: "sensor.temperature", show_config_targets: false });
      await el.updateComplete;
    });

    describe("WHEN rendered", () => {
      it("THEN no dp-chip-group is shown", () => {
        expect(el.shadowRoot.querySelector("dp-chip-group")).toBeNull();
      });
    });
  });
  ...
  describe("GIVEN the static config methods", () => {
    describe("WHEN getStubConfig is called", () => {
      it("THEN it returns default config", () => {
        expect(HassRecordsActionCard.getStubConfig()).toEqual({ title: "Record Event" });
      });
    });
    ...
    describe("WHEN getConfigElement is called", () => {
      it("THEN it returns the editor element tag", () => {
        ...
        expect(editorEl.tagName.toLowerCase()).toBe("hass-datapoints-action-card-editor");
      });
    });
  });
