 ...
 * formatting helpers remain in `lib/chart/chart-shell.js`.
 */
...
export function renderChartAxisOverlays(card, renderer, axes = []) {
  ...
  const chartWrap = getRoot(card)?.querySelector(".chart-wrap") ?? card;
  if (chartWrap) {
...
export function renderChartAxisHoverDots(card, hoverValues = []) {
  const leftEl = getRoot(card)?.getElementById("chart-axis-left");
  const rightEl = getRoot(card)?.getElementById("chart-axis-right");
  const scrollViewport = getRoot(card)?.getElementById("chart-scroll-viewport");
