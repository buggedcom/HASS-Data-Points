...
import { localizeText, syncFrontendLocale } from "@/lib/i18n/localize";
import {
  confirmDestructiveAction,
  ensureHaComponents,
} from "@/lib/ha/ha-components.js";
import { PANEL_HISTORY_SAVED_PAGE_KEY } from "@/lib/data/preferences-api.js";
import {
...
export class HassRecordsHistoryPanel extends HTMLElement {
  constructor() {
    ...
    this._onToggleEventVisibility = (ev) =>
      this._handleToggleEventVisibility(ev);
    this._onToggleSeriesVisibility = (ev) =>
    ...
    this._onCollapsedSidebarClick = (ev) =>
      this._handleCollapsedSidebarClick(ev);
  ...
  _initFromContext() {
    ...
    if (datapointsScopeFromUrl === "all") {
      ...
      !datapointsScopeFromUrl &&
      sessionState?.datapoint_scope === "all"
    ) {
      resolvedDatapointScope = "all";
    } else if (
      !datapointsScopeFromUrl &&
      sessionState?.datapoint_scope === "hidden"
    ) {
      resolvedDatapointScope = "hidden";
    } else {
    ...
    this._showChartDatapointIcons =
      sessionState?.show_chart_datapoint_icons !== false;
    this._showChartDatapointLines =
      sessionState?.show_chart_datapoint_lines !== false;
    this._showChartTooltips = sessionState?.show_chart_tooltips !== false;
    this._showChartEmphasizedHoverGuides =
      sessionState?.show_chart_emphasized_hover_guides === true;
    this._chartHoverSnapMode =
      sessionState?.chart_hover_snap_mode === "snap_to_data_points"
        ? "snap_to_data_points"
        : "follow_series";
    ...
    this._showCorrelatedAnomalies =
      sessionState?.show_chart_correlated_anomalies === true;
    this._chartAnomalyOverlapMode = ANALYSIS_ANOMALY_OVERLAP_MODE_OPTIONS.some(
      (o) => o.value === sessionState?.chart_anomaly_overlap_mode
    )
    ...
    this._dataGapThreshold = DATA_GAP_THRESHOLD_OPTIONS.some(
      (option) => option.value === sessionState?.data_gap_threshold
    )
    ...
    if (Object.keys(targetFromUrl).length) {
      ...
      nextTargetSelection = normalizeTargetValue(
        sessionState.target_selection_raw ||
          sessionState.target_selection || { entity_id: sessionState.entities }
      );
    } else {
    ...
    this._targetSelectionRaw =
      !hasTargetInUrl && sessionState?.target_selection_raw
        ? sessionState.target_selection_raw
        : nextTargetSelection;
    this._seriesRows =
      !hasTargetInUrl && Array.isArray(sessionState?.series_rows)
    ...
    const zoomStart =
      ...
      (!zoomStartFromUrl && !zoomEndFromUrl
        ? parseDateValue(sessionState?.zoom_start_time)
        : null);
    const zoomEnd =
      parseDateValue(zoomEndFromUrl) ||
      (!zoomStartFromUrl && !zoomEndFromUrl
        ? parseDateValue(sessionState?.zoom_end_time)
        : null);
    this._chartZoomRange = null;
    ...
    if (Number.isFinite(hoursFromUrl) && hoursFromUrl > 0) {
      ...
      !hasRangeInUrl &&
      Number.isFinite(sessionState?.hours) &&
      sessionState.hours > 0
    ) {
  ...
  _buildLoadingShell() {
    this._shellBuilt = false;
    const loadingLabel = localizeText("Loading Datapoints…");
  ...
  _buildShell() {
    ...
    this.shadowRoot.innerHTML = `<style>${PANEL_HISTORY_STYLE}</style>`;
  ...
  _applyPreferredSeriesColors(rows, urlColorMap = null) {
    const queryColors =
      urlColorMap && typeof urlColorMap === "object" ? urlColorMap : {};
  ...
  _syncHassBindings() {
    ...
    this.shadowRoot
      ...
      .forEach((iconEl) => {
        const entityId =
          iconEl.dataset.seriesIconEntityId ||
          iconEl.dataset.seriesCollapsedIconEntityId;
        if (!entityId) {
          return;
        }
  ...
  _renderSidebarOptions() {
    ...
    this._sidebarOptionsComp.anyAnomaliesEnabled = (
      this._seriesRows ?? []
    ).some((r) => r.analysis?.show_anomalies === true);
  ...
  _getPreviewComparisonWindows() {
    ...
    const previewWindows = comparisonIds
      .map(
        (id) =>
          this._comparisonWindows.find((window) => window.id === id) || null
      )
      .filter(Boolean)
      .map((window) => ({
        ...window,
    ...
    if (this._hoveredComparisonWindowId) {
      return (
        this._comparisonWindows.find(
          (window) => window.id === this._hoveredComparisonWindowId
        ) || null
      );
    }
    if (this._selectedComparisonWindowId) {
      return (
        this._comparisonWindows.find(
          (window) => window.id === this._selectedComparisonWindowId
        ) || null
      );
    }
    return null;
  ...
  _createDateWindowFromDialog(overrides = {}) {
    ...
    const parsedStart = overrides.start
      ? this._parseDateWindowInputValue(overrides.start)
      : null;
    const parsedEnd = overrides.end
      ? this._parseDateWindowInputValue(overrides.end)
      : null;
    const start =
  ...
  _handleComparisonTabHover(id) {
    this._context.orchestration.handleComparisonTabHover({
      ...
      setHoveredComparisonWindowId: (value) => {
  ...
  _handleComparisonTabActivate(id) {
    this._context.orchestration.handleComparisonTabActivate({
      ...
      setSelectedComparisonWindowId: (value) => {
  ...
  _openTargetPicker() {
    this._context.orchestration.openTargetPicker(this._targetControl);
  }
  ...
  _mountHistoryTargetsControl() {
    ...
    histTargets.addEventListener("dp-targets-add-click", () => {
      this._openTargetPicker();
    });
    histTargets.addEventListener("dp-collapsed-entity-click", (ev) => {
      ...
      if (this._collapsedPopupEntityId === entityId) {
        ...
        this._showCollapsedTargetPopup(entityId, buttonEl);
      }
    });
  ...
  _refreshCollapsedTargetPopup() {
    ...
    const row = this._seriesRows.find(
      (r) => r.entity_id === this._collapsedPopupEntityId
    );
    if (!row) {
      this._hideCollapsedTargetPopup();
      return;
  ...
  _refreshCollapsedOptionsPopup() {
    ...
    const popup = this._shellEl?.getOptionsPopupEl();
    const menu = popup?.querySelector("collapsed-options-menu");
    if (menu) {
      this._syncCollapsedOptionsMenu(menu);
    }
  }
  ...
  _syncCollapsedOptionsMenu(menu) {
    ...
    menu.anyAnomaliesEnabled = (this._seriesRows ?? []).some(
      (r) => r.analysis?.show_anomalies === true
    );
  }
  ...
  _updateSeriesRowVisibilityByEntityId(entityId, visible) {
    ...
    const index = this._seriesRows.findIndex(
      (row) => row.entity_id === normalizedEntityId
    );
    if (index === -1) {
      return;
    }
  ...
  _handleChartZoom(ev) {
    const start = Number.isFinite(ev?.detail?.startTime)
      ? ev.detail.startTime
      : null;
  ...
  _updateUrl({ push = false } = {}) {
    this._context.navigation.updateUrl({
      ...
      seriesColorQueryKey: (entityId) => this._seriesColorQueryKey(entityId),
      push,
    });
  }
  ...
  _renderComparisonTabs() {
    const result = this._context.orchestration.renderComparisonTabs({
      ...
      formatComparisonLabel: (startTime, endTime) =>
