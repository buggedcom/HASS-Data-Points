...
import { setupCanvas } from "@/chart-utils";
...
describe("setupCanvas", () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(canvas, "getContext").mockReturnValue({ scale: vi.fn() } as unknown as CanvasRenderingContext2D);
  });
  ...
  describe("WHEN cssHeight is very small", () => {
    it("THEN canvas.height is at least 120 CSS pixels", () => {
      expect.assertions(1);
      Object.defineProperty(window, "devicePixelRatio", { value: 1, configurable: true });
      setupCanvas(canvas, container, 50, 400);
